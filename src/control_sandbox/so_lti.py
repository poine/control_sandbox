...
import logging, timeit, pickle
import math, numpy as np, scipy.signal, scipy.integrate, matplotlib.pyplot as plt

import control, pdb

LOG = logging.getLogger('so_lti')

import misc_utils as ut, plot_utils as pu
...
def plot2(time, X, U=None, Yc=None, figure=None, window_title="trajectory"):
    ...
    figure = pu.prepare_fig(figure, window_title, figsize=(0.5*20.48, 0.5*10.24), margins=margins)
    ...
    if U is not None:
        ...
        plt.plot(time, U)
        pu.decorate(ax, title="$u$", xlab='time')
    return figure
...
def plot(time, X, U=None, Yc=None):
    ...
    pu.decorate(ax, title="$x_1$")
    ax = plt.subplot(3,1,2)
    plt.plot(time, X[:,1])
    pu.decorate(ax, title="$x_2$")
    if U is not None:
