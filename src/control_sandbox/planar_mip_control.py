...
def sim_state_feedback(X0, K, H, P):
    sp = lambda t: step(t, a=0.4)
...
def sim_place(X0, poles):
    ...
    time, X, U = sim_state_feedback(X0, K, H, P)
    return time, X, U, P, 'Place'
...
def sim_two_loops_ctl(X0=[0, 0, 0, 0], sp=lambda t: step(t, a=0.9)):
    P = pmip.Param()
...
def main(save_anim=False):
    ...
    if 0:
        poles = [-5, -3.5+2.j, -3.5-2.j, -20]
        time, X, U, P, exp_name = sim_place(X0, poles)
    if 0:
        #Q, R = np.diag([10, 1, 1, 1]), np.diag([2])
        Q, R = np.diag([10, 1, 0.1, 0.01]), np.diag([2])
        time, X, U, P, exp_name =  sim_lqr(X0, Q, R, step)
    if 1:
        poles = [-5, -3.5+2.j, -3.5-2.j, -20]
        time, X, U, P, exp_name =  sim_two_loops_ctl(X0, sp=lambda t: step(t, a=0.2))
    #pmip_u.plot(time, X, U, window_title=exp_name)
    #anim = pmip_u.animate(time, X, U, P, exp_name)
    anim = pmip_u.animate_and_plot(time, X, U, P, exp_name, _drawings=True, _imgs=True)
    if save_anim:
